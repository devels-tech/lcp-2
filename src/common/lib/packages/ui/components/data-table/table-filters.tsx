...
export const DataTableFilters = ({ columns, tempFilters, setTempFilters, tempSearch, setTempSearch, open, setOpen, setOpenFilters, onSubmit }) => {
  ...
  return (
    <div className='flex items-center justify-between py-4'>
      <FilterContainer open={open} onOpenChange={setOpen}>
        ...
        <FilterContainerContent className={cn(isDesktop ? 'max-w-sm' : 'w-full px-4')} side='left'>
          ...
          <section className={cn(isDesktop ? 'w-full h-[70svh]' : 'w-full h-full max-h-[50svh] overflow-y-auto')}>
            ...
            <div className='space-y-4'>
              {filterColumns?.map?.((column) => (
                <div key={column.id as string} className='space-y-2'>
