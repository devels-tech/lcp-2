...
const TSidebarContext = React.createContext<SidebarContext | null>(null)

function useSidebarUI() {
  const context = React.useContext(TSidebarContext)
  if (!context) {
...
const SidebarProvider = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'> & { defaultOpen?: boolean, open?: boolean, onOpenChange?: (open: boolean) => void }>(
  (
    ...
    return (
      <TSidebarContext.Provider value={contextValue}>
